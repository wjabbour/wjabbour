...
|---|---|---|---|---|---|
| 09/22/2026 | [#58055](https://github.com/vllm-project/vllm/pull/58055) | vllm-project/vllm | Wire 17 root-level `tests/test_*.py` into CI, fix 2 stale tests found | Follow-up to #54867; running these tests for the first time in a long time surfaced a real assertion bug in `version.py` and a broken `caplog` assumption in a platform-detection test, both fixed | Draft (9 PRs already open, opened as draft to skip the 8-PR cap) |
| 09/06/2026 | [#5297](https://github.com/ROCm/aiter/pull/5297) | ROCm/aiter | Route `asm_mla.cu` ctypes entrypoint failures through the error bridge instead of `std::abort` | An unsupported MLA-decode shape (e.g. spec-decode verify length > 4 on MI300X) raised `std::abort` and killed the whole worker; now it's a catchable `RuntimeError` a caller can handle | Awaiting review (validated on MI300X) |
| 09/06/2026 | [#55611](https://github.com/vllm-project/vllm/pull/55611) | vllm-project/vllm | Exercise `ROCM_AITER_FA` and `FLASH_ATTN` MLA prefill backends in AMD CI (fixes #53944) | The ROCm MLA prefill backends had almost no CI coverage (every case skip-marked); adds real `FLASH_ATTN` prefill correctness plus two optional AITER-FA jobs | Draft — validated on MI300X, awaiting a slot to mark ready |
...
|---|---|---|---|---|
| 09/22/2026 | [#54867](https://github.com/vllm-project/vllm/pull/54867) | vllm-project/vllm | Pre-commit check that new tests are tethered to a Buildkite job | Fails a PR that adds a `tests/` file no CI job collects, so a test can no longer merge and then silently never run |
